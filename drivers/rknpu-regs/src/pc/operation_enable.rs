...
#[doc = "Field `OP_EN` reader - PC 操作使能。0：禁用 PC 模块；1：使能 PC 模块，为每个 task 取寄存器配置"]
pub type OpEnR = crate::BitReader;
#[doc = "Field `OP_EN` writer - PC 操作使能。0：禁用 PC 模块；1：使能 PC 模块，为每个 task 取寄存器配置"]
pub type OpEnW<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - PC 操作使能。0：禁用 PC 模块；1：使能 PC 模块，为每个 task 取寄存器配置"]
    #[inline(always)]
