...
pub fn probe_all_devices() -> Vec<super::AxDeviceEnum> {
    ...
    #[cfg(feature = "block")]
    {
        ...
        warn!("[DBG] block device count from rdrive: {}", ls.len());
        for dev in ls {
            let mut dev_blk = crate::dyn_drivers::blk::Block::from(dev);
            warn!("[DBG] probing block dev: {}", dev_blk.device_name());
            if dev_blk.is_gpt_partition() {
                warn!("[DBG] -> is GPT: true");
                let root = "rootfs".parse().unwrap();
                // let root = "Linux data partition".parse().unwrap();
                match GptPartitionDev::try_new(dev_blk, |_, part| {
                    warn!("[DBG]   checking partition: {}", part.name);
                    part.name == root
                }) {
                    Ok(d) => {
                        warn!("[DBG] -> Found GPT partition 'rootfs': {}", d.device_name());
                        devices.push(super::AxDeviceEnum::from_block(d));
                    }
                    Err(e) => {
                        warn!("[DBG] -> Failed to find GPT partition 'rootfs': {e}");
                        // devices.push(super::AxDeviceEnum::from_block(dev_blk));
                    }
                }
            } else {
                warn!("[DBG] -> is GPT: false, pushing raw block dev");
                devices.push(super::AxDeviceEnum::from_block(dev_blk));
            }
        }
    }
