...
本周是 RKNN NPU 驱动开发的第一周，主要补充了npu的基础知识学习、开发环境搭建、闭源库逆向文档整理，以及驱动骨架的初步编写。
...
## 2. RKNN Toolkit2 套件与仿真验证

上 Rockchip 官网下载了 [rknn-toolkit2](https://github.com/airockchip/rknn-toolkit2) 套件，发现里面包含一个 **仿真器**，可以在 x86 主机上模拟 RKNPU 的部分功能。这意味着不需要开发板也能跑通部分测例。

阅读了 demo 代码，理解了 RKNN 推理的基本 API 调用流程：
...
搭载 RK3588 芯片的 **OrangePi 5 Plus** 开发板到了之后，将带有 NPU 支持的 StarryOS 版本（[atomgit 仓库](https://atomgit.com/aios-porting/3906cf9f2ccbf898bd3512d5862ef92e)）部署到开发板上，**成功跑通测例**。

这验证了 NPU 驱动在 StarryOS 上的可行性，也为后续开发提供了参考实现。
...
最初尝试直接在 `arceos/modules/` 下创建 `axnpu-rknn` 驱动模块，但遇到了较多的**循环依赖问题**（`axdriver → axfs → axmm → axdma → axdriver`），
